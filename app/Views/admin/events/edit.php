...
<div class="max-w-2xl">
    ...
    <form method="POST" action="<?= url('/admin/evenements/' . $event['id']) ?>"
        ...
        <div>
            ...
            <input name="title" required value="<?= e($event['title']) ?>" class="form-input w-full">
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <select name="type" class="form-input w-full">
                    ...
                    <option value="<?= e($k) ?>" <?= $event['type'] === $k ? 'selected' : '' ?>><?= e($v) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div>
                ...
                <select name="discipline" class="form-input w-full">
                    ...
                    <option value="<?= e($k) ?>" <?= $event['discipline'] === $k ? 'selected' : '' ?>><?= e($v) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div>
                ...
                <input type="datetime-local" name="start_datetime" required
                       value="<?= e($toInput($event['start_datetime'])) ?>" class="form-input w-full">
            </div>
            <div>
                <label class="form-label">Fin</label>
                <input type="datetime-local" name="end_datetime"
                       value="<?= e($toInput($event['end_datetime'])) ?>" class="form-input w-full"></div>
            <div class="sm:col-span-2">
                <label class="form-label">Lieu</label>
                <input name="location" value="<?= e($event['location']) ?>" class="form-input w-full"
        ...
        <div>
            ...
            <textarea name="description" rows="4" class="form-input w-full"><?= e($event['description']) ?></textarea>
        </div>

        <label class="flex items-center gap-2 text-sm font-montserrat">
            <input type="checkbox" name="is_published" value="1" <?= $event['is_published'] ? 'checked' : '' ?>>
            Publié
        </label>
