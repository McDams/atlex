...
<article class="max-w-3xl mx-auto px-4 sm:px-6 py-16">
    ...
    <div class="flex items-center gap-3 mt-6 mb-4">
        <span class="text-xs font-montserrat uppercase tracking-wide px-3 py-1 rounded bg-atlex-red text-white">
            <?= e(news_category_label($article['category'])) ?>
        </span>
        <span class="text-white/40 text-sm">
            <?= e(format_date_fr($article['published_at'] ?? $article['created_at'])) ?>
        </span>
    </div>

    <h1 class="font-bebas text-4xl sm:text-5xl tracking-wide leading-tight mb-6">
        <?= e($article['title']) ?>
    </h1>
    ...
    <?php if (!empty($article['excerpt'])): ?>
        <p class="text-lg text-atlex-beige font-montserrat italic mb-6">
            <?= e($article['excerpt']) ?>
        </p>
    <?php endif; ?>

    <div class="article-content text-white/80">
        <?= $article['content'] ?>
    </div>
</article>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <?php foreach ($related as $r): ?>
                ...
                <a href="<?= url('/actualites/' . $r['slug']) ?>" class="block bg-atlex-bg rounded-xl p-5 border border-white/5 hover:border-atlex-red/40 transition-colors">
                    <span class="text-xs text-atlex-red font-montserrat uppercase">
                        <?= e(news_category_label($r['category'])) ?>
                    </span>
                    <h3 class="font-montserrat font-bold mt-1">
                        <?= e($r['title']) ?>
                    </h3>
                </a>
            <?php endforeach; ?>
